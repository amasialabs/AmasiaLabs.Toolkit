...
public static class JwtBearerExtensions
{
    ...
    /// Uses configuration keys: "Jwt:Issuer", "Jwt:Audience" and either "JwtKey" or "Jwt:Key".
    ...
    private static (string Issuer, string Audience, string Key) ResolveJwtSettings(IConfiguration configuration)
    {
        ...
        // Try root-level "JwtKey" first (as commonly used), then fallback to "Jwt:Key"
        var key = configuration["JwtKey"] ?? jwtSection["Key"];
        ...
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException("JWT signing key is missing. Provide 'JwtKey' or 'Jwt:Key'.");
